...
/***************************************************************************************
**函数名:       POST_CalcLen
**功能描述:     邮电部协议计算数据长度。注意编译器是大小端。IAR环境下测试通过。
**输入参数:     u16 len
**输出参数:     符合邮电部协议的数据长度。
**备注:         YY.c,2017-06-23,By Haixing.Huang
****************************************************************************************/
u16 POST_CalcLen(u16 len)
{
  ...
  uniTmp.tmp=len;
  uniTmp.tmp4bit.d = (~((uniTmp.tmp4bit.a+uniTmp.tmp4bit.b+uniTmp.tmp4bit.c)%16)&0x000F)+1;  //根据邮电部协议获得。s
...
{ 		  
  ...
  for(i=0;i<len;i++)
  {
    USART3_sendchar(DaPtr[i]);//打印回复数据
  }PrintString_("");//换行
  ...
  {	//判断出呼叫本机 6=SOI+EOI+SUM==4 ASCII码
    ...
    {  	 
      ...
      //len = len/2 + 1;//重新定义长度//error
      PrintString_("进行格式转换");//换行
      ...
      {		 				    
//        if((Var==0x11)||(Var==0x20))//协议版本
        {	 
          if(!ChkPostDataSum(DaPtr,len))
          {  
            RTN=0;	
            if((DaPtr[4]==0)||(DaPtr[4]>6))//判断接收的RTN是正常回复（RTN=1~6为发送格式错误的反馈，直接丢弃）
              DealPostPro(uf,DaPtr,len);            //
            else{
              PrintString("平台回复——RTN:");
