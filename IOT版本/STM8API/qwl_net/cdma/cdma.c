...
#define AT_CPIN	        "AT+CPIN?"		//查询SIM卡状态
#define AT_CIPSTART     "at+zipsetup=0,"//GPRS连接方式\GPRS连接地址(根据用户实际IP修改)\GPRS连接的端口
#define AT_CIPCLOSE     "at+zpppclose"	//TCP连接关闭
#define AT_zpppopen     "at+zpppopen"		//打开网络
#define AT_zpppstatus   "at+zpppstatus"//查询连接状态
#define AT_CIPSEND      "at+zipsend=0,"		//GPRS发送信息命令
#define AT_GETICCID     "AT+GETICCID"		//查询SIM卡号
#define AT_CIMI         "AT+CIMI"		//查询SIM卡号
  ...
  case 3:
    {
      if(net.delay>2)//等待超时，服务器问题
      {
        net.delay=0;
      net.setp_setp=2;
      PrintString_("查sim卡状态超时");
      net.delay_times++;
      if(net.delay_times>5)
      {PrintString_("sim卡不存在");
      net.delay_times=0;
      net.setp_setp=0;
      net.reconnect_setp=MODE_CHOICE;//进入网络重新连接
      net.delay=0;
...
void cdma_reconnect()
{
  ...
  if(Net_Data_Send.Link_Type == UPLOAD)
    TP_GPRS(AT_CIPSTART,Sys_keydata.update_server_add);
      ...
      TP_GPRS(AT_CIPSTART,Sys_keydata.default_cloud_server_add);//连接IP
      net.setp_setp=1;
      net.delay=0;
  ...
  case 4:
    {
      ...
      if(strsearch_h("OPENED",SystemBuf)>0)
      {
        net.reconnect_setp=OPEN_GPRS;//打开完毕，进入联网
        net.setp_setp=5;
        net.delay_times=0;
        net.delay=0;
        PrintString_("打开网络连接成功");
      }
...
u8 cdma_send(u8 *p,u16 len)
{
  ...
  u16 g_delay_ms=0;
  clear_SystemBuf();
  qwl_net_sendstring1(AT_CIPSEND);   //发送 "at+zipsend=0,"		//GPRS发送信息命令
  j=len;
  if(len>1000) 
  {
